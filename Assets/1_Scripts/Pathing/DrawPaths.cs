    ...
    public class DrawPaths : MonoBehaviour
    {
        ...
        {            
            for (int i = 0; i < points.Length; i++)
            {
                ...
                float cylinderDistance = 0.5f * Vector3.Distance(this.points[i].transform.position, this.points[j].transform.position);
                this.ringGameObjects[i].transform.localScale = new Vector3(this.ringGameObjects[i].transform.localScale.x, cylinderDistance, this.ringGameObjects[j].transform.localScale.z);
                ...
                this.ringGameObjects[i].transform.LookAt(this.points[j].transform, Vector3.up);
                this.ringGameObjects[i].transform.rotation *= Quaternion.Euler(90, 0, 0);
            }
        }

        public void DrawingPathLines(bool isEnabled)
        {
            foreach( var ringObject in ringGameObjects)
            {
                ringObject.GetComponentInChildren<MeshRenderer>().enabled = isEnabled;
            }
        }
